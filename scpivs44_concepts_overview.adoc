...
---
= SnapCenter Plug-in for VMware vSphere에 대해 알아보기
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
NOTE: NetApp Backup and Recovery를 사용하여 VMware 보호 기능을 강화하십시오. 이 솔루션은 3-2-1 백업, 랜섬웨어 보호, 자동화된 백업 관리, 향상된 확장성, 비용 절감, 그리고 단일 인터페이스에서 여러 vCenter 인스턴스에 걸쳐 데이터스토어와 VM의 중앙 집중식 백업 관리를 제공합니다. SCV 정책, 스냅샷 및 일정을 NetApp Backup and Recovery로 마이그레이션하고 단일 콘솔에서 모든 VM 보호를 관리할 수 있습니다. 자세한 내용은 link:https://docs.netapp.com/us-en/data-services-backup-recovery/["NetApp Backup and Recovery 설명서"]를 참조하십시오.

====
SnapCenter Plug-in for VMware vSphere는 VM 및 데이터스토어에 대한 데이터 보호 서비스를 제공하고, SnapCenter 애플리케이션 기반 플러그인에 대한 데이터 보호 서비스를 지원하는 독립형 가상 어플라이언스(Open Virtual Appliance 형식)입니다. 이 문서는 SnapCenter Plug-in for VMware vSphere를 배포하고 사용하는 방법과 빠른 시작 정보를 설명합니다. SnapCenter Plug-in for VMware vSphere는 Linux 기반 가상 어플라이언스로 배포됩니다.

VMware vSphere용 SnapCenter 플러그인은 귀사의 환경에 다음과 같은 기능을 추가합니다.

* VM 일관성 및 충돌 시에도 정합성 보장 데이터 보호 작업 지원
+
vCenter의 VMware vSphere 클라이언트 사용자 인터페이스를 사용하여 VMware 가상 머신(기존 VM 및 vVol VM), VMDK 및 데이터 저장소의 모든 백업 및 복원 작업을 수행할 수 있습니다.  vVol VM(vVol 데이터 저장소의 VM)의 경우 충돌 일관성 백업만 지원됩니다.  VM과 VMDK를 복원하고 게스트 OS에 있는 파일과 폴더도 복원할 수 있습니다.
...
+
VM의 기본 및 보조 스토리지에 있는 데이터베이스와 파일 시스템의 모든 백업 및 복원 작업에 SnapCenter 사용자 인터페이스와 적절한 SnapCenter 애플리케이션 플러그인을 사용할 수 있습니다.
...
+
SnapCenter 데이터베이스와 VM의 단일 스냅샷을 함께 지원하지 않습니다.  VM과 데이터베이스에 대한 백업은 독립적으로 예약하고 실행해야 하므로 데이터베이스와 VM이 동일한 볼륨에 호스팅되어 있더라도 별도의 스냅샷이 생성됩니다.  SnapCenter 사용자 인터페이스를 사용하여 데이터베이스 애플리케이션 백업을 예약하고, VMware vSphere 클라이언트 사용자 인터페이스를 사용하여 VM 및 데이터 저장소 백업을 예약합니다.
...
+
자세한 내용은 ONTAP tools for VMware vSphere 설명서를 참조하세요. 또한 다음을 참조하세요. https://imt.netapp.com/imt/imt.jsp?components=180121;&solution=1517&isHWU&src=IMT["NetApp 상호 운용성 매트릭스 툴"^] ONTAP 도구에서 지원되는 버전에 대한 최신 정보를 확인하세요.
...
지원되는 버전에 대한 최신 정보는 다음을 참조하세요. https://imt.netapp.com/imt/imt.jsp?components=180121;&solution=1517&isHWU&src=IMT["NetApp 상호 운용성 매트릭스 툴"^] (IMT).
