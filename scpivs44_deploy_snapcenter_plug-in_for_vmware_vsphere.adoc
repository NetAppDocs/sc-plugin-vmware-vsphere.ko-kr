...
---
= SnapCenter Plug-in for VMware vSphere를 배포합니다
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
* vVol 데이터스토어에서 VM을 보호하려면 먼저 ONTAP tools for VMware vSphere 배포하세요. 지원되는 ONTAP 도구 버전은 다음을 참조하세요. https://imt.netapp.com/imt/imt.jsp?components=180121;&solution=1517&isHWU&src=IMT["NetApp 상호 운용성 매트릭스 툴"^] . ONTAP 도구는 ONTAP 및 VMware 웹 클라이언트에서 스토리지를 프로비저닝하고 구성합니다.
