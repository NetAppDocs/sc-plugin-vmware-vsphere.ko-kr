...
---
= SnapCenter Plug-in for VMware vSphere 배포 후 필요한 작업 및 문제
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
+
** IPv4 주소를 실행하고 있으며 SnapCenter VMware vSphere 호스트에 대한 IP 주소가 두 개 있습니다. 따라서 작업 요청이 SnapCenter 서버에서 인식하지 못하는 IP 주소로 전송됩니다. 이 문제를 방지하려면 다음과 같이 사용할 IP 주소를 추가합니다.
+
...
. 브라우저 캐시를 지운 다음 사용자 인터페이스가 제대로 작동하는지 확인하세요.
+
문제가 지속되면 VMware vSphere Client 서비스를 다시 시작합니다

. vCenter에 로그인한 다음 도구 모음에서 * 메뉴 * 를 선택한 다음 * SnapCenter Plug-in for VMware vSphere * 를 선택합니다.
