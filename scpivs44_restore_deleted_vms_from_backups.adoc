...
+
특정 백업 이름 또는 일부 백업 이름을 검색하거나 필터 아이콘을 클릭하고 날짜 및 시간 범위를 선택하고 VMware 스냅샷이 포함된 백업을 사용할지 여부, 마운트된 백업을 사용할지 여부 및 위치를 선택하여 백업 목록을 필터링할 수 있습니다. 확인 * 을 클릭하여 마법사로 돌아갑니다.

. 범위 선택 * 페이지의 * 복구 범위 * 필드에서 * 전체 가상 머신 * 을 선택한 다음 복구 위치를 선택한 다음 백업을 마운트할 대상 ESXi 호스트 정보를 입력합니다.
+
복구 대상은 SnapCenter에 추가된 ESXi 호스트일 수 있습니다. 이 옵션은 VM이 지정된 시간 및 날짜를 사용하여 스냅숏에서 상주하는 선택한 백업의 내용을 복원합니다. 이 옵션을 선택하면 * Restart VM *(VM 다시 시작 *) 확인란이 선택되어 있고 VM의 전원이 켜집니다.
