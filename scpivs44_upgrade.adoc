...
NOTE: SCV 6.2로의 업그레이드는 VMware vCenter Server 7 업데이트 1 이상 버전에서만 지원됩니다. VMware vCenter Server 7 업데이트 1 이전 버전을 사용하는 경우 SCV 4.7을 계속 사용해야 합니다. 지원되지 않는 버전의 VMware vCenter Server에서는 업그레이드 시 문제가 발생할 수 있습니다.
...
CAUTION: 업그레이드하기 전에 사용 환경이 시스템 요구 사항을 충족하는지 확인하십시오. 자세한 내용은 link:scpivs44_deployment_planning_and_requirements.html["SnapCenter Plug-in for VMware vSphere 배포 계획 및 요구 사항"]을 참조하십시오.
...
지원되는 버전에 대한 최신 정보는 link:https://imt.netapp.com/imt/imt.jsp?components=180121;&solution=1517&isHWU&src=IMT["NetApp 상호 운용성 매트릭스 툴(IMT)"^]을 참조하십시오.
...
+
.. link:https://mysupport.netapp.com/products/index.html["NetApp 지원 사이트"^]에 로그인하십시오.
